...
def convert_tensor_to_list(data):
    if isinstance(data, torch.Tensor):
        ...
        return {k: convert_tensor_to_list(v) for k, v in data.items()}  # 递归处理字典
    elif isinstance(data, list):
        return [convert_tensor_to_list(item) for item in data]  # 递归处理列表
    else:
        return data  # 返回非 Tensor 类型的值
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        serializable_res = convert_tensor_to_list(res)
        output_file = '/media/vipsl-1/Domain_adaption_repo/jyq/soma_2/results.json'
        ...
        # 合并新数据
        existing_data.append(serializable_res)

        # 写回文件（覆盖原有内容）
        with open(output_file, 'w') as f:
            json.dump(existing_data, f, indent=4)  # 使用 indent=4 使输出格式更美观
