...
class AOODDetection(VisionDataset):
    ...
    def get_aood_settings_pascal_voc(self, setting=1):
        ...
        CLASS_NAMES = tuple(itertools.chain(BASE_CLASSES, UNK))    #获得包含基类和unknown的类别名
        return BASE_CLASSES, NOVEL_CLASSES, ALL_CLASSES, CLASS_NAMES
    ...
    def __init__(self,
        ...
        #使用列表推导式，遍历 file_names 中的每个文件名 x，将其与 img_folder 拼接，并添加上确定的后缀 suffix，构建完整的图像路径。
        self.annotations.extend([os.path.join(ann_folder, x + ".xml") for x in file_names])
        #使用列表推导式，遍历 file_names，将每个文件名 x 与 ann_folder 拼接，并添加上 .xml 后缀，构建完整的注释文件路径。
        self.imgids = list(range(len(file_names)))
        self.imgids2img = dict(zip(self.imgids, file_names))            #将0-N序号与图像文件名一一对应
        self.imgid2annotations.update(dict(zip(self.imgids, self.annotations)))    #将0-N序号与标注一一对应
    ...
    def load_instances(self, img_id):
        ...
        for obj in target['annotation']['object']:
            ...
            bbox[1] -= 1.0
            instance = dict(
                    category_id=self.ALL_CLASSES.index(cls),    #这里的ID是对于所有的类的，类别3并不是unknown类
                    bbox=bbox,
